        ...
        if (Input.GetKeyDown(KeyCode.T))
        {
            if (scannerManager != null && scannerManager.GetTrackedObjects() != null && scannerManager.GetTrackedObjects().Count > 0)
            {
                ...
                for (int i = 0; i < scannerManager.GetTrackedObjects().Count; i++)
                {
                    ...
                    {
                        if (Vector3.Distance(scannerManager.GetTrackedObjects()[i].transform.position, transform.position) < Vector3.Distance(nearestObject.transform.position, transform.position))
                        {
